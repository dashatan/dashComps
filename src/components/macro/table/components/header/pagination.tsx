...
import { ChangeTag, TableData, constants } from "../../types";
...
export const Pagination = memo(({ onchange, loading }: PaginationProps) => {
  ...
  return (
    <Paginator
      ...
      pageLinkSize={5}
      className="dir-ltr h-14"
      onPageChange={handleChange}
      nextPageLinkIcon={<Icon icon="ArrowDown" className="-rotate-90 scale-75 [&_*]:fill-slate-500" />}
      prevPageLinkIcon={<Icon icon="ArrowDown" className="rotate-90 scale-75 [&_*]:fill-slate-500" />}
...
function DoubleArrow(props: React.DetailedHTMLProps<React.HTMLAttributes<HTMLDivElement>, HTMLDivElement>) {
  return (
    <div {...props} className={classNames("flex items-center justify-center [&_*]:fill-slate-500", props.className)}>
      <Icon icon="ArrowDown" className="rotate-90 scale-75 -mr-4" />
